...
class GenderClassifier(KerasClassifier):
    ...
    def transform(self, x, samplerate):
        x = pad_sequences([x], maxlen=40000, padding='post', dtype=float, truncating='post')[0]
        to_process = np.mean(mfcc(x, samplerate), axis=0)
        to_process = pad_sequences([to_process], maxlen=249, padding='post')[0]
        return to_process
